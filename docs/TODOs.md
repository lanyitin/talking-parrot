...
- [ ] `implement-vad-stage`：實作 `VADStage`，整合 `TenVADBackend`、`SileroVADBackend`，並以 `FormulaEvaluator` 計算 composite score，輸出 `VadSegment` 清單。
- [ ] `implement-chunking-stage`：實作 `ChunkingStage`，貪婪合併 VadSegment 至 max_chunk_seconds，含 silence_pad 與超長硬切兜底邏輯。
- [ ] `implement-transcription-stage`：實作 `TranscriptionStage`，整合 `FasterWhisperBackend`（Windows/Linux）與 `MLXWhisperBackend`（macOS），並以 `ConditionEvaluator` 實現級聯升級邏輯。
